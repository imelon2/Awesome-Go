...
func main() {
	...
	join := filepath.Join(root, "nitro-contracts", "build", "contracts", "src", "*", "*.sol", "*.json")
	filePaths, err := filepath.Glob(filepath.Join(root, "nitro-contracts", "build", "contracts", "src", "*", "*.sol", "*.json"))
	if err != nil {
		log.Fatal(err)
	}
